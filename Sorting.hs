...
-- find the nth largest element in constant time

-- quick select, with O(n^2) worse case, but average n(log(n)), like quicksort 


-- find the min element in constant time
-- isMin :: Ord a => a -> [a] -> Boolean
-- isMin 

-- find the max element in constant time
